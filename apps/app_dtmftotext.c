...
static int get_input_text(struct ast_channel *chan, const char *variable_name, const char *initial_digits, int max_chars, int max_time)
{
    ...
    struct ast_frame *f;
    const struct ast_format *original_read_fmt = ast_channel_readformat(chan);
    ...
    if (ast_format_cmp(original_read_fmt, ast_format_slin) == AST_FORMAT_CMP_NOT_EQUAL)
    {
    ...
    if (ast_format_cmp(original_read_fmt, ast_format_slin) == AST_FORMAT_CMP_NOT_EQUAL)
    {
        res = ast_set_read_format(chan, (struct ast_format *)original_read_fmt);
        if (res)
