...
/*** DOCUMENTATION
	<application name="FFMPEG" language="en_US">
		<synopsis>
		...
		<description>
			...
			exten => 1234,1,FFMPEG(/var/lib/asterisk/playlist.m3u)
...
static int ff_exec(struct ast_channel *chan, const char *data)
{
	...
	next = ast_tvnow();
	next.tv_sec += 1;
	ast_log(LOG_NOTICE, "FFMPEG spawned, starting to read STDIN");
	if (res >= 0) {
		pid = res;
