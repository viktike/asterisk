...
static struct ast_amqp_connection *amqp_connection_reconnect(struct ast_amqp_connection *cxn)
{
    struct ast_amqp_connection *new_cxn;
    const char *name = ast_strdupa(cxn->name);  // Copy name since we'll release cxn
    
    ast_debug(3, "Attempting to reconnect AMQP connection %s\n", name);
    
    /* Create new connection with same name */
    new_cxn = amqp_connection_create(name);
    if (!new_cxn) {
        ast_log(LOG_ERROR, "Failed to reconnect AMQP connection %s\n", name);
        return NULL;
    }
    
    /* Replace old connection in active_connections */
    ao2_lock(active_connections);
    ao2_unlink_flags(active_connections, cxn, OBJ_NOLOCK);
    ao2_link_flags(active_connections, new_cxn, OBJ_NOLOCK);
    ao2_unlock(active_connections);
    
    return new_cxn;  // Caller gets a reference
}
...
{
    int retry_count = 0;
    const int max_retries = 3;  /* Maximum number of reconnection attempts */
    ...
    while (retry_count < max_retries) {
        ...
        case AMQP_STATUS_TCP_ERROR:
            ...
            ast_debug(2, "AMQP connection lost (error: %d), attempting reconnect\n", res);
            
            struct ast_amqp_connection *new_cxn = amqp_connection_reconnect(cxn);
            if (!new_cxn) {
                ast_log(LOG_ERROR, "Reconnection failed\n");
                return -1;
            }
            
            /* Update the original connection object */
            ao2_lock(cxn);
            amqp_connection_state_t old_state = cxn->state;
            cxn->state = new_cxn->state;
            new_cxn->state = NULL;  /* Transfer ownership of state */
            ao2_unlock(cxn);
            
            /* Clean up */
            amqp_destroy_connection(old_state);
            ao2_ref(new_cxn, -1);
            
            retry_count++;
            continue;  /* Retry publish with new connection */
    ...
    ast_log(LOG_ERROR, "Failed to publish after %d reconnection attempts\n", 
            max_retries);
    return -1;
}
