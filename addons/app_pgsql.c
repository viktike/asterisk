...
static const char app[] = "PGSQL";

static const char synopsis[] = "Do several pgSQLy things";

static const char descrip[] = 
"PGSQL():  Do several pgSQLy things\n"
"Syntax:\n"
...
static int aPGSQL_connect(struct ast_channel *chan, char *data) {
...
static int aPGSQL_query(struct ast_channel *chan, char *data) {
...
static int aPGSQL_fetch(struct ast_channel *chan, char *data) {
...
static int aPGSQL_reset(struct ast_channel *chan, char *data) {
...
static int aPGSQL_clear(struct ast_channel *chan, char *data) {
...
static int aPGSQL_disconnect(struct ast_channel *chan, char *data) {
...
static int aPGSQL_debug(struct ast_channel *chan, char *data) {
	ast_log(LOG_WARNING,"Debug : %s\n",data);
	return(0);
}

static int PGSQL_exec(struct ast_channel *chan, const char *data)
{
	int result=0;
	...
	if (!data) {
		...
		ast_debug(5, "PGSQL: data=%s\n", data);
	}
	
	if (strncasecmp("connect",data,strlen("connect"))==0) {
		result=(aPGSQL_connect(chan,ast_strdupa(data)));
	} else 	if (strncasecmp("query",data,strlen("query"))==0) {
		result=(aPGSQL_query(chan,ast_strdupa(data)));
	} else 	if (strncasecmp("fetch",data,strlen("fetch"))==0) {
		result=(aPGSQL_fetch(chan,ast_strdupa(data)));
	} else 	if (strncasecmp("reset",data,strlen("reset"))==0) {
		result=(aPGSQL_reset(chan,ast_strdupa(data)));
	} else 	if (strncasecmp("clear",data,strlen("clear"))==0) {
		result=(aPGSQL_clear(chan,ast_strdupa(data)));
	} else  if (strncasecmp("debug",data,strlen("debug"))==0) {
		result=(aPGSQL_debug(chan,ast_strdupa(data)));
	} else 	if (strncasecmp("disconnect",data,strlen("disconnect"))==0) {
		result=(aPGSQL_disconnect(chan,ast_strdupa(data)));
	} else {
...
AST_MODULE_INFO_STANDARD(ASTERISK_GPL_KEY, "Simple Pgsql Interface");
