...
"  MYSQL(Connect connid dhhost[:dbport]/dbsock dbuser dbpass dbname [dbcharset])\n"
...
static int aMYSQL_connect(struct ast_channel *chan, const char *data)
{
	...
	if(strchr(args.dbhost, '/')!=NULL){
		if (!mysql_real_connect(mysql, NULL, args.dbuser, args.dbpass, args.dbname, 0, args.dbhost,
